...
fn indexed_delegate_no_todo() {
    ...
    "#;
    let (_header, c_source) = generate_c(source, "idxdel");
    assert!(
        !c_source.contains("TODO"),
        "indexed delegate should not contain TODO in generated C code, got:\n{}",
        c_source,
    );
}
